...
document.addEventListener('DOMContentLoaded',function(){
  ...
  tryRestoreSnapshot();
  var pinBtn=document.getElementById('pinUnlockBtn');if(pinBtn)pinBtn.addEventListener('click',function(){var ok=unlockApp();if(!ok){var inp=document.getElementById('pinInput');if(inp)inp.focus();}});
  var pinInput=document.getElementById('pinInput');if(pinInput){pinInput.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();var ok=unlockApp();if(!ok)pinInput.select();}});setTimeout(function(){if(!window.appUnlocked&&pinInput)pinInput.focus();},50);}
  uiLogic();
});
})();
  `;
  ...
  return (
    <>
      <Link href="/dashboard" className="dashboard-tab">Dashboard →</Link>
